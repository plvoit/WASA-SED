    ...
    IF (istate==0) THEN
        ...
        DO WHILE (istate==0)
            SELECT CASE (trim(locase(dummy)))		!enable/disable file output of desired results
                ...
                CASE ('snowenergycont')
                    f_snowEnergyCont=.TRUE. .AND. (dosnow)
                CASE ('snowwaterequiv')
                    f_snowWaterEquiv=.TRUE. .AND. (dosnow)
                CASE ('snowalbedo')
                    f_snowAlbedo=.TRUE. .AND. (dosnow)
                CASE ('snowcover')
                    f_snowCover=.TRUE. .AND. (dosnow)
                CASE ('snowtemp')
                    f_snowTemp=.TRUE. .AND. (dosnow)
                CASE ('surftemp')
                    f_surfTemp=.TRUE. .AND. (dosnow)
                CASE ('liqufrac')
                    f_liquFrac=.TRUE. .AND. (dosnow)
                CASE ('fluxprec')
                    f_fluxPrec=.TRUE. .AND. (dosnow)
                CASE ('fluxsubl')
                    f_fluxSubl=.TRUE. .AND. (dosnow)
                CASE ('fluxflow')
                    f_fluxFlow=.TRUE. .AND. (dosnow)
                CASE ('fluxnets')
                    f_fluxNetS=.TRUE. .AND. (dosnow)
                CASE ('fluxnetl')
                    f_fluxNetL=.TRUE. .AND. (dosnow)
                CASE ('fluxsoil')
                    f_fluxSoil=.TRUE. .AND. (dosnow)
                CASE ('fluxsens')
                    f_fluxSens=.TRUE. .AND. (dosnow)
                CASE ('stoiprec')
                    f_stoiPrec=.TRUE. .AND. (dosnow)
                CASE ('stoisubl')
                    f_stoiSubl=.TRUE. .AND. (dosnow)
                CASE ('stoiflow')
                    f_stoiFlow=.TRUE. .AND. (dosnow)
                CASE ('ratealbe')
                    f_rateAlbe=.TRUE. .AND. (dosnow)
                CASE ('precipmod')
                    f_precipMod=.TRUE. .AND. (dosnow)
                CASE ('cloudfrac')
                    f_cloudFrac=.TRUE. .AND. (dosnow)
                CASE ('radimod')
                    f_radiMod=.TRUE. .AND. (dosnow)
                CASE ('temperamod')
                    f_temperaMod=.TRUE. .AND. (dosnow)
                CASE ('rel_elevation')

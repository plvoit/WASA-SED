    ...
    DO WHILE (.TRUE.) !read till end of file
        ...
        if (which1(luse_subbas(i) == id_subbas_extern) == 0) then
            ...
            READ(cdummy,*,IOSTAT=istate) luse_subbas(i),luse_lu(i),luse_tc(i),  &
                rocky(i),nbr_svc(i),(id_soil_intern(c,i),c=1,dummy1)    !!ii: nbr_svc, rocky, id_soil_intern sind feststehende Parameter fÃ¼r einen TC-Typ und sollten nur einmal pro TC-typ gespeichert werden
            k=k+istate
...
!            end if
    ...
    if (doloadstate .OR. dosavestate .OR. dosediment .OR. doirrigation) then
        ...
        READ(11,*)  !----2 Zeilen Ã¼berspringen, dann wird die Anzahl der SVC ermittelt, also Im Prinzip die Anzahl der Zeilen
        READ(11,*)
...
if (dosnow) then
    ...
    IF (file_exists) THEN
        ...
        rule_options(3) = "cwd"   ! crop water demand
        ...
        allocate(sub_source(l), irri_source(l), sub_receiver(l), irri_rule(l), irri_rate_gw(subasin+1,subasin, 4), irri_rate_riv(subasin+1,subasin, 4), &  ! subasin + 1 because of option external
                                 ...
                                 cwd_gw(subasin+1,subasin+1,4), cwd_ext(subasin+1,4), cwd_riv(subasin+1,subasin+1,4), cwd_res(subasin+1,subasin+1,4), cwd_lake(subasin+1,subasin+1,4), &
    ...
    !-------------Checks, Errors and Warnings-----------
            ...
            ! Unfortunately this doesn't work because the reservoir index doesn't exist yet. this check happens no on daily basis in irrigation_abstraction.f90. Same for lake check
            ...
            if (irri_rule(j) == "cwd" .AND. sub_receiver(j) == 9999) then
                WRITE(*,'(a, I0, a)') 'WARNING (irri.dat): Line ',h,': When using rule "cwd", sub_receiver can not be external (9999). Line ignored.'
                cycle
            end if

            if (irri_rule(j) == "cwd") then
                season_filecheck = PACK(seasonality_irri(:,1), MASK= seasonality_irri(:,1) == sub_receiver(j))
                if (SIZE(season_filecheck) == 0) then
                    WRITE(*,'(a, I0, a, I0, a)') 'WARNING (irri.dat): sub_receiver in line ',h,' not contained in irri_seasons.dat. To use rule "cwd" receiver basin must be included in irri_seasons.dat. Line ignored'
                    cycle
                end if
            end if
            ...
            !---- Sort irri.dat entries in respective arrays to be then used in irrigation_abstraction.f90
             ...
             elseif (irri_source(j) == "groundwater" .AND. sub_receiver(j) /= 9999 .AND. irri_rule(j) /= "cwd") then
                irri_rate_gw(sub_receiver(j),sub_source(j), 1:4) = irri_rate(1:4) !assign read rates to the respective source array (gw, res, lake, etc.)
             endif

             if (irri_source(j) == "reservoir" .AND. sub_receiver(j) == 9999 .AND. irri_rule(j) /= "cwd") then
                irri_rate_res(subasin+1,sub_source(j), 1:4) = irri_rate(1:4)
             else if (irri_source(j) == "reservoir" .AND. sub_receiver(j) /= 9999 .AND. irri_rule(j) /= "cwd") then
                irri_rate_res(sub_receiver(j),sub_source(j), 1:4) = irri_rate(1:4) !assign read rates to the respective source array (gw, res, lake, etc.)
             endif

             if (irri_source(j) == "lake" .AND. sub_receiver(j) /= 9999 .AND. irri_rule(j) /= "cwd") then
             ...
             else if (irri_source(j) == "lake" .AND. sub_receiver(j) == 9999 .AND. irri_rule(j) /= "cwd") then
                irri_rate_lake(subasin+1,sub_source(j), 1:4) = irri_rate(1:4)
             end if

             if (irri_source(j) == "river" .AND. sub_receiver(j) /= 9999 .AND. irri_rule(j) /= "cwd" ) then
             ...
             else if (irri_source(j) == "river" .AND. sub_receiver(j) == 9999 .AND. irri_rule(j) /= "cwd") then
                irri_rate_riv(subasin+1,sub_source(j), 1:4) = irri_rate(1:4)
             end if

             if (irri_source(j) == "9999" .AND. sub_receiver(j) /= 9999 .AND. irri_rule(j) /= "cwd") then
             ...
             else if (irri_source(j) == "9999" .AND. sub_receiver(j) == 9999 .AND. irri_rule(j) /= "cwd") then
             ...
             if (irri_source(j) == "river" .AND. irri_rule(j) == "cwd" .AND. sub_source(j) /= 9999) then
                cwd_riv(sub_receiver(j),sub_source(j),:) = irri_rate(1:4)
             else if (irri_source(j) == "river" .AND. irri_rule(j) == "cwd" .AND. sub_source(j) == 9999) then
                cwd_riv(sub_receiver(j),subasin+1,:) = irri_rate(1:4)
             end if

             if (irri_source(j) == "reservoir" .AND. irri_rule(j) == "cwd" .AND. sub_source(j) /= 9999) then
                cwd_res(sub_receiver(j),sub_source(j),:) = irri_rate(1:4)
             else if (irri_source(j) == "reservoir" .AND. irri_rule(j) == "cwd" .AND. sub_source(j) == 9999) then
                cwd_res(sub_receiver(j),subasin+1,:) = irri_rate(1:4)
             end if

             if (irri_source(j) == "lake" .AND. irri_rule(j) == "cwd" .AND. sub_source(j) /= 9999) then
                cwd_lake(sub_receiver(j),sub_source(j),:) = irri_rate(1:4)
             else if (irri_source(j) == "reservoir" .AND. irri_rule(j) == "cwd" .AND. sub_source(j) == 9999) then
                cwd_lake(sub_receiver(j),subasin+1,:) = irri_rate(1:4)
             end if
